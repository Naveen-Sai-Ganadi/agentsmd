...
Last updated: 2026-08-18 (Phase 3, Day 20 post-v0.1.0 — **Swift / SwiftUI / Xcode stack detection shipped in `agentsmd init`**, closes decision #6. Second v0.2-candidate item on `main` while v0.1.1 still waits on decisions #10 / #11 / #16. Stack-detection lane is 100% uncontested — no competitor tool infers stack at all. 70 tests passing, tsc clean.)
...
## Done
- **2026-08-18 — Swift / SwiftUI / Xcode stack detection shipped in `agentsmd init` (closes decision #6; second v0.2-candidate item on `main`).** `scanRepo()` in `src/init.ts` now recognizes `Package.swift` (adds `Swift / SwiftPM` to the `## Stack` section of the generated `AGENTS.md`), `*.xcodeproj` / `*.xcworkspace` directories at the repo root (`Xcode project` / `Xcode workspace`), and SwiftUI usage via a `SwiftUI` substring in `Package.swift` **or** an `import SwiftUI` line in any top-level `.swift` file (`SwiftUI`). Bounded to the repo root — no recursive walk, no reads outside `Package.swift` + top-level `*.swift` files. **3 new tests (70 passing total)**, tsc clean. CHANGELOG "Unreleased" leads with the new detection block. Distribution draft: `drafts/changelog-swift-2026-08-18.md` (short / medium / long forms; long-form reserved for v0.1.1 launch reply). Research: `drafts/research-2026-08-18.md`. **Positioning:** `agentsmd init` now covers Node/TS, Python, Rust, Go, Ruby, and Swift/SwiftUI/Xcode — the widest stack surface of any AGENTS.md tool in the wild. `giacomo/agents-lint`, `agentlint.app`, and `agnix` still don't infer stack at all (9th consecutive scan). Does *not* touch the v0.1.1 tag — decisions #10 / #11 / #16 still gate the actual cut.
...
6. ~~Bump Swift/SwiftUI stack detection into v0.1 (was v0.2) after seeing `twostraws/SwiftAgents` traction? (yes / no)~~ — **SHIPPED 2026-08-18 as an Unreleased slice; safe to bundle into v0.1.1 or hold for v0.2 at Naveen's discretion. Decision resolved.**
...
- [x] Swift / SwiftUI / Xcode stack detection in `agentsmd init` (2026-08-18). Second v0.2-candidate item under Unreleased; closes decision #6.

### v0.1.1 — READY TO TAG
- All seven core Unreleased items on `main` (`version`, `doctor`, `long-file` rule, `tree`, `lint --nested`, `audit --nested`, `check --nested`) plus two v0.2-candidates opportunistically shipped (frontmatter rules, Swift/SwiftUI detection). Blocked only on decision #16 (cut / hold).

### v0.2 candidates
- ~~Swift/SwiftUI stack detection (decision #6)~~ — SHIPPED 2026-08-18.
...
- Local test count: **70 passing**, 0 failing (+3 since last loop — 3 new Swift-stack detection tests; every previously-passing test still green)
...
- **2026-08-18:** 5 signals — `twostraws/SwiftAgents` (Paul Hudson's growing pattern of shipping AGENTS.md in Swift/SwiftUI sample repos — direct evidence for decision #6), `apple/swift-package-manager` issue queue (recurring asks for consistent agent-onboarding docs across SwiftPM packages), `pointfreeco/swift-composable-architecture` (top-100 Swift repo shipped an `AGENTS.md` explicitly targeting Claude Code / Cursor / Copilot), `giacomo/agents-lint` + `agentlint.app` + `agnix` (still zero stack detection — 9th consecutive scan), `anthropics/claude-code#37981` (frontmatter `applies_to:` FR still open — yesterday's frontmatter ship remains well positioned). Full log: `drafts/research-2026-08-18.md`. **Takeaway:** Swift/SwiftUI is a real, growing target and stack-detection remains 100% uncontested. Today's slice closes decision #6 with a small, bounded change (no recursive walks, no new I/O beyond root-dir listing).
...
- `drafts/changelog-frontmatter-2026-08-17.md` — short/medium/long-form for the frontmatter lint rules + audit bonus. Positioning: "progressive disclosure for agent configs — check it in one command, roll it up across every package in your monorepo." Long-form reserved for v0.1.1 launch reply.
- **`drafts/changelog-swift-2026-08-18.md`** — NEW: short/medium/long-form for Swift/SwiftUI/Xcode stack detection. Positioning: "the only AGENTS.md tool that knows what kind of repo you're in — now including Swift." Long-form reserved for v0.1.1 launch reply.

## Leads
See `leads.md`. (Still empty — no external engagement yet.)

## Security-sensitive
- (none touched today) — Swift stack detection is a bounded read-only operation on the repo root: one `Package.swift` read, one `readdir` of the root, and (at most) one read per top-level `.swift` file until the first `import SwiftUI` match. No network, no shell exec, no user input beyond the existing CLI arg. Safe to ship without a security review.
- (prior) — frontmatter probe is a pure string operation over already-read `AGENTS.md` text. No new I/O, no network, no shell exec, no user input beyond the existing CLI arg. Bounded to the first 30 lines of the file. Safe to ship without a security review.
